...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            attachedObject.originalScene = objectToAttach.scene;
        ...
        static void RestoreDetachedObjectScene(AttachedObject attachedObject)
        {
            if (attachedObject.attachedObject == null)
                return;

            GameObject detached = attachedObject.attachedObject;
            if (detached.scene.name != "DontDestroyOnLoad")
                return;

            Scene targetScene = attachedObject.originalScene;
            if (attachedObject.originalParent != null)
                targetScene = attachedObject.originalParent.scene;

            if (!targetScene.IsValid())
                targetScene = SceneManager.GetActiveScene();

            if (targetScene.IsValid() && detached.scene != targetScene)
                SceneManager.MoveGameObjectToScene(detached, targetScene);
        }
        ...
        public void DetachObject(GameObject objectToDetach, bool restoreOriginalParent = true)
        {
            ...
            if (index != -1)
            {
                ...
                RestoreDetachedObjectScene(attachedObjects[index]);
